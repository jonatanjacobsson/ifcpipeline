#!/usr/bin/env bash
# run-phase10-block-cold.sh - like run-phase10-block.sh, but drops the
# kernel page cache before every campaign so the .so + input.ifc start
# cold each time. Eliminates the "host warmed up" confound in the rate.
#
# Usage:
#   bash run-phase10-block-cold.sh <label_base> <K> [N=8] [M=3] [ROUNDS=2]
...
for i in $(seq 1 "${K}"); do
    # Clear cores and page cache.
    echo "${SUDO_PASS}" | sudo -S sh -c '
        rm -f /var/crash/cores/core-*
        sync
        echo 3 > /proc/sys/vm/drop_caches
    ' || true

    LABEL="${LABEL_BASE}-${i}"
    echo "######## ${LABEL} (campaign ${i}/${K}) cold-start ########"
...
echo
echo "================= BLOCK SUMMARY: ${LABEL_BASE} (cold) ================="
