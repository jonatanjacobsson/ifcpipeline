...
echo ""
echo "==> Named volumes (expect postgres-data, seaweedfs-data, redis-data under ${PROJECT}_*)"
docker volume ls --format '{{.Name}}' | grep -E "${PROJECT}|postgres|seaweed|redis" | sort || true
echo ""
