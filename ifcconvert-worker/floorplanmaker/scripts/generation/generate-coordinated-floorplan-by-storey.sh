#!/bin/bash
# =============================================================================
# Generate Coordinated Floor Plans (All Disciplines) - By Storey Name
# =============================================================================
# Generates a single floor plan with all disciplines layered using storey-based
# element selection instead of section heights:
...
#   6. Spaces (labels)
#
# Elements are selected based on their assigned building storey, not section cuts.
#
# Usage:
#   ./generate-coordinated-floorplan-by-storey.sh "020 Mezzanine +5.40m"
#
# =============================================================================
...
echo ""
echo "==========================================="
echo "COORDINATED FLOOR PLAN GENERATION (BY STOREY)"
echo "==========================================="
echo ""
...
# Step 1: Export all layers using storey-based selection
echo "Step 1: Exporting layers by storey assignment..."

# Layer 1: Architecture (underlay)
echo "  [1/6] Exporting architecture underlay..."
...
# Layer 6: Spaces (labels)
echo "  [6/6] Exporting spaces..."
docker exec ifcpipeline-ifcconvert-worker-1 /usr/local/bin/IfcConvert \
  -y -j 8 -q \
  --model \
  --include+ attribute Name "$STOREY_NAME" \
...
echo "Step 2: Combining 6 layers with proper opacities and scaling..."
...
try:
    ...
    # Load all 6 layers with metadata
    layer_config = [
...
if [ -f "/home/bimbot-ubuntu/apps/ifcpipeline/shared${OUTPUT_FILE}" ]; then
    ...
    echo "  Size: $SIZE"
    echo "  Layers: 6 (Architecture, Structural, MEP, Spaces)"
    echo "    - Selection method: By storey assignment"
    echo "    - Storey: $STOREY_NAME"
    echo "  Scale: 1:50"
    
    # Clean up intermediate files (optional)
    # rm -f "$ARCH_LOCAL" "$STRUCT_LOCAL" "$PLUMB_LOCAL" "$MECH_LOCAL" "$ELEC_LOCAL" "$SPACES_LOCAL"
fi
